...
//        Broadcast<SmallHilbertCurve> smallHilbertCurveBr = jsc.<SmallHilbertCurve>broadcast(hilbertCurve);
        ...
        String fullPathExportedFile = metricsPath+ File.separator+"frechet-queries-distances-try4-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt";
...
//                        trSegments.forEach(t->trackletsCellIds.add(t._1));
//
