...
import gr.ds.unipi.spatialnodb.messages.common.trajparquet.TrajectorySegmentWithMetadataReadSupport;
...
public class Frechet2DQueriesDirectoriesIntervalsPivotsVar1 {
...
//        Broadcast<SmallHilbertCurve> smallHilbertCurveBr = jsc.<SmallHilbertCurve>broadcast(hilbertCurve);
        ...
        String fullPathExportedFile = metricsPath+ File.separator+"frechet-"+epsilon+"-"+"pivots-var1-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt";
...
//                }
            ...
            JavaPairRDD<Long, TrajectorySegmentWithMetadata> pairRDDRangeQuery = null;

            if(sbFullyCovers.length() != 0){
                sbFullyCovers.deleteCharAt(sbFullyCovers.length()-1);
                pairRDDRangeQuery = (JavaPairRDD<Long, TrajectorySegmentWithMetadata>) jsc.newAPIHadoopFile(sbFullyCovers.toString(), ParquetInputFormatWithKey.class, Long.class, TrajectorySegmentWithMetadata.class, jobFullyContains.getConfiguration());
            }

            if(sbIntersected.length()!=0){
                sbIntersected.deleteCharAt(sbIntersected.length()-1);
                JavaPairRDD<Long, TrajectorySegmentWithMetadata> intersectedPairRDD = (JavaPairRDD<Long, TrajectorySegmentWithMetadata>) jsc.newAPIHadoopFile(sbIntersected.toString(), ParquetInputFormatWithKey.class, Long.class, TrajectorySegmentWithMetadata.class, jobIntersected.getConfiguration());
...
//            if(sbIntersected.length()==0 && sbFullyCovers.length()!=0){System.exit(5);}
...
//                }
            ...
            JavaPairRDD<Void, TrajectorySegment> results = pairRDDRangeQuery.groupBy(f->f._2().getTrajectorySegment().getObjectId(), Integer.parseInt(args[0]))
                    .flatMapToPair(f->{
                        ...
                        List<Tuple2<Long,TrajectorySegmentWithMetadata>> trSegments = new ArrayList<>();
                        f._2.forEach(trSegments::add);

                        Comparator<Tuple2<Long,TrajectorySegmentWithMetadata>> comparator = Comparator.comparingLong(d-> d._2.getInterval()[0]);
