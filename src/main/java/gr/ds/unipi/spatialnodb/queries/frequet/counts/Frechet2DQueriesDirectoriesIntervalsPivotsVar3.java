...
public class Frechet2DQueriesDirectoriesIntervalsPivotsVar3 {
...
//        Broadcast<SmallHilbertCurve> smallHilbertCurveBr = jsc.<SmallHilbertCurve>broadcast(hilbertCurve);
        ...
        String fullPathExportedFile = metricsPath+ File.separator+"prunings-frechet-queries-pivots-var3-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt";
...
//                                sb.append(parquetPath+ File.separator+"stIndex"+File.separator+r+",");
            ...
            pairRDDRangeQuery = (JavaPairRDD<Long, TrajectorySegmentWithMetadata>) jsc.newAPIHadoopFile(sb.toString(), ParquetInputFormatWithKey.class, Long.class, TrajectorySegmentWithMetadata.class, job.getConfiguration());
    ...
    public static JavaPairRDD<Void, TrajectorySegment> completeTrajectories(JavaPairRDD<Long, TrajectorySegmentWithMetadata> results){
