...
public class ExaminedTrajectoriesKnnQueries {
...
//            List<Double> varMinDist = new ArrayList<>();
            ...
            while (!EQ.isEmpty() || !IQ.isEmpty()) {
                ...
                if (iqKey <= eqKey) {
                    ...
                    JavaPairRDD<Void, TManRecord> ex = candidates.filter(f -> {
                            ...
                            checkedTrajectories += (int) ex.count();

                    JavaPairRDD<Double, TManRecord> fetchedTrajectories = ex.mapToPair(f -> Tuple2.apply(HilbertUtil.frechetDistance(trajectoryQuery, f._2.getSpatialPoints()), f._2)).filter(f -> f._1 <= finalEpsilon);
